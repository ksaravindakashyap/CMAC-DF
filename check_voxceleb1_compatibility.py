"""Compatibility check for VoxCeleb1 with the visual RIR pipeline.

This script samples a small number of VoxCeleb1 video clips, runs the existing
visual RIR pipeline, and summarizes whether the dataset is suitable for the
...
LOGGER = logging.getLogger("check_voxceleb1_compatibility")
...
def discover_videos(dataset_root: str, extensions: Iterable[str] = VIDEO_EXTENSIONS) -> list[Path]:
    """Find candidate VoxCeleb1 video clips beneath a dataset root.

    Args:
        dataset_root: Root folder containing VoxCeleb1 clips.
...
def run_compatibility_check(dataset_root: str, output_dir: str, sample_count: int = 100, seed: int = 7) -> list[SampleResult]:
    """Run the VoxCeleb1 compatibility test on a sample of clips.

    Args:
        dataset_root: Root directory containing VoxCeleb1 video clips.
    ...
    if not sampled_videos:
        LOGGER.warning("No VoxCeleb1 video clips were found under %s", root)
...
def _write_outputs(results: list[SampleResult], out_dir: Path) -> None:
    ...
    summary_path = out_dir / "voxceleb1_compatibility_summary.json"
    records_path = out_dir / "voxceleb1_compatibility_records.csv"
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Compatibility check for VoxCeleb1 with the visual RIR pipeline")
    parser.add_argument("--dataset_root", required=True, help="Root directory of the VoxCeleb1 video clips")
    parser.add_argument("--output_dir", default="./voxceleb1_check", help="Directory for output artifacts")
